...
#include "MoskitoCoaxialHeat_1p_MC.h"

registerMooseObject("MoskitoApp", MoskitoCoaxialHeat_1p_MC);

template <>
InputParameters
validParams<MoskitoCoaxialHeat_1p_MC>()
{
    ...
    params.addRequiredCoupledVar("salinity_inner_pipe", "salinity inside inner pipe (Molar)");
    ...
    params.addRequiredCoupledVar("salinity_outer_pipe", "salinity inside outer pipe (Molar)");
...
MoskitoCoaxialHeat_1p_MC::MoskitoCoaxialHeat_1p_MC(const InputParameters & parameters)
  : Material(parameters),
    eos_uo(getUserObject<MoskitoEOS1P_MC>("eos_uo")),
...
void
MoskitoCoaxialHeat_1p_MC::computeQpProperties()
{
  Real j = 0.0 ;
