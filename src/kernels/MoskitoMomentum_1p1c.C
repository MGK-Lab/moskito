...
MoskitoMomentum_1p1c::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  if (jvar == _m_var_number)
  {
    ...
    j *= _u[_qp] * _u[_qp];
    j += 2.0 * _drho_dm[_qp] * _phi[_j][_qp] * _u[_qp] * _grad_u[_qp];
    j += _well_sign[_qp] * _f[_qp] * _drho_dm[_qp] * _phi[_j][_qp] * _u[_qp]
          * _u[_qp] * _perimeter[_qp] * _well_dir[_qp] / (8.0 * _area[_qp]);
    j /= _area[_qp] * _area[_qp];
    j -= _drho_dm[_qp] * _phi[_j][_qp] * _gravity[_qp];
    j *= _test[_i][_qp];
  }
