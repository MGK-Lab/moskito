...
template <>
InputParameters
validParams<MoskitoTimeEnergy_1p1c>()
{
...
MoskitoTimeEnergy_1p1c::computeQpJacobian()
{
  ...
  j += (_drho_dp[_qp] * _p_dot[_qp] + _drho_dT[_qp] * _u_dot[_qp] + _drho_dm[_qp] * _m_dot[_qp]) * _cp[_qp] * _phi[_j][_qp];
...
MoskitoTimeEnergy_1p1c::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  if (jvar == _m_var_number)
  {
    ...
    j *= (_h[_qp] + _q[_qp] * _q[_qp] / (2.0 * _area[_qp] * _area[_qp]));
    j += _drho_dm[_qp] * _phi[_j][_qp] * (_cp[_qp] * _u_dot[_qp] + _q[_qp] * _q_dot[_qp] / (_area[_qp] * _area[_qp]));
    j *= _test[_i][_qp];
  }
